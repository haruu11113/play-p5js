...
void main() {
    ...
    float noiseValue = smoothFractalNoise(dir * noiseScale + vec3(uTime * timeScale) + noiseOffset);
    ...
    // 二次的な変形を追加（より滑らかな形状に）
    float secondaryNoise = smoothFractalNoise(dir * noiseScale * 1.5 + vec3(uTime * timeScale * 1.2));
    // sinを使って滑らかな変化を追加
    ...
    // cosを使って滑らかな変化を作る
    float bumpFactor = 0.25 * pow(max(0.0, dot(normalize(dir), normalize(bumpDirection))), 2.0);
    radius += bumpFactor;
    
    // 変形した位置を計算
    vec3 distortedPosition = dir * radius;
    ...
    // 方向ベクトルに直交する接線を計算
    if (abs(dot(dir, tangentX)) > 0.99) {
        tangentX = vec3(0.0, 1.0, 0.0);
        tangentY = vec3(0.0, 0.0, 1.0);
    } else {
        tangentX = normalize(cross(dir, vec3(0.0, 1.0, 0.0)));
        tangentY = normalize(cross(dir, tangentX));
    }
    
    // 微小変位を使って接線方向の点を計算
    vec3 nearbyX = normalize(dir + epsilon * tangentX);
    vec3 nearbyY = normalize(dir + epsilon * tangentY);
    ...
    vec3 newNormal = normalize(cross(tangentVecX, tangentVecY));
